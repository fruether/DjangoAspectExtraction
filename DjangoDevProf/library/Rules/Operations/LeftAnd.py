...
class LeftAnd(LogicalOperation):

    def __init__(self, filter1, filter2):
        super(LeftAnd, self).__init__(filter1, filter2)
    def __init__2(self, filter1, filter2):
        super(LeftAnd, self).__init__(filter1, filter2, -1)
    """
    First calls super function to get the values and then performs the AND logic
    """
    def execute(self, *args):
        result1, result2 = super(LeftAnd, self).execute(*args)
