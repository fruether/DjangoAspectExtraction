...
class RightOr(LogicalOperation):

    def __init__(self, filter1, filter2):
        super(RightOr, self).__init__(filter1, filter2)

    def __init__2(self, filter1, filter2):
        super(RightOr, self).__init__(filter1, filter2, None)

    """
    First calls super function to get the values and then performs the OR logic
    """
    def execute(self, *args):
        result1, result2 = super(RightOr, self).execute(*args)
